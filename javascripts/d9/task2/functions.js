...
const  displayInfo = function (nameArr ,gradeArr){

  const arrlength = nameArr.length;
  if(checkGrade&&checkName){
    for(let i = lastindex; i<arrlength; i++){
      studentRow = document.createElement("tr");

      tableObject.append(studentRow);
      

      const nameData = document.createElement("td");
      nameData.textContent =nameArr[i];
      studentRow.append(nameData);

      const gradData = document.createElement("td");
      gradData.textContent =gradeArr[i];
      studentRow.append(gradData);
      const checkedBox = document.querySelector('input[name=Department:checked').value;
      if(checkedBox=="SD"){
        studentRow.style.backgroundColor="blue";
      }else if(checkedBox=="OS"){
        studentRow.style.backgroundColor="green";
      }else if(checkedBox=="OS"){
        studentRow.style.backgroundColor="red";
      }else {
        studentRow.style.backgroundColor="cyan";
      }
      deleteIcon = document.createElement("td");
      deleteIcon.textContent="Delete";
      deleteStudent (deleteIcon);
      console.log("row");
...
const deleteStudent = function(deleteObj){

  deleteObj.addEventListener("click",function(event){
